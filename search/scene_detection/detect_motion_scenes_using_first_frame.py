...
def detect_scene_with_motion_first_frame(input_path, output_path, border_msec,
    ...
    # initialize the background template frame as the first frame in the video stream
    first_frame = None
    ...
    while True:
        ...
        frame = utils.resize(frame, width=600)
        ...
        # compute the difference between the current frame and first frame
        ...
        if dsw.recording and consec_frames == args["buffer_size"]:
            dsw.finish()
            dynamic_scene_boundaries_list.append(video_msec)
